...
# Initialize DB
def init_db():
    conn = sqlite3.connect("RO_GeoData.db")
    c = conn.cursor()
    c.execute("""
    CREATE TABLE IF NOT EXISTS stops (
        ...
        bezirk TEXT,
        gemeinde TEXT,
        distance_trainstation REAL,
        distance_mountain REAL,
        distance_minigolf REAL,
        distance_museum REAL
    )
    """)
    c.execute("""
...
def store_stops_within_boundary(stops, boundary, conn):
    ...
    for stop in stops:
        ...
        if boundary.contains(pt):
            ...
            c.execute("SELECT COUNT(*) FROM stops WHERE name = ?", (stop_name,))
            if c.fetchone()[0] > 0:
                continue  # Skip adding this stop if a duplicate name exists
            
            c.execute("""
                INSERT OR IGNORE INTO stops (id, name, lat, lon)
                VALUES (?, ?, ?, ?)
            """, (stop['id'], stop_name, stop['lat'], stop['lon']))
    conn.commit()
...
def store_boundaries(conn, boundary, bezirke, gemeinden):
    ...
    c.execute("""
    INSERT INTO boundaries (name, type, shape_data)
    VALUES (?, ?, ?)
...
def annotate_stops(conn, bezirke, gemeinden):
    c = conn.cursor()
    c.execute("SELECT id, lat, lon FROM stops")
    for stop_id, lat, lon in c.fetchall():
        pt = Point(lon, lat)
...
def clean_stop_names(conn):
    c = conn.cursor()
    c.execute("SELECT id, name, gemeinde FROM stops WHERE gemeinde IS NOT NULL")
    updates = 0
    for stop_id, name, gemeinde in c.fetchall():
        ...
        if name.lower().startswith(gemeinde.lower()):
            ...
            if new_name:  # Avoid empty name after removal
                c.execute("UPDATE stops SET name = ? WHERE id = ?", (new_name, stop_id))
                updates += 1
    conn.commit()
...
def store_routes_and_stops_routes(routes, stops_routes, conn):
    ...
    for route_id, route_name in routes.items():
        c.execute("""
        INSERT OR IGNORE INTO routes (id, name)
        VALUES (?, ?)
        """, (route_id, route_name))
    ...
    for stop_id, route_id in stops_routes:
        ...
        c.execute("SELECT 1 FROM stops WHERE id = ?", (stop_id,))
        if c.fetchone():  # Stop exists
            c.execute("""
            INSERT OR IGNORE INTO stops_routes (stop_id, route_id)
            VALUES (?, ?)
            """, (stop_id, route_id))
...
def update_altitudes(conn):
    c = conn.cursor()
    c.execute("SELECT id, lat, lon FROM stops WHERE altitude IS NULL")
    stops = c.fetchall()
...
def remove_routes_with_few_stops(conn):
    ...
    SELECT r.id
    FROM routes r
    LEFT JOIN stops_routes sr ON r.id = sr.route_id
    GROUP BY r.id
    ...
    if routes_to_remove:
        ...
        c.executemany("""
        DELETE FROM stops_routes WHERE route_id = ?
        """, [(route_id,) for route_id in routes_to_remove])
    ...
    for poi_type, tag_variations in poi_types.items():
        ...
        for element in unique_elements:
            ...
            if boundary.contains(pt):
                ...
                c.execute("""
                INSERT OR IGNORE INTO points_of_interest (name, lat, lon, type)
                VALUES (?, ?, ?, ?)
...
def calculate_distances(conn, step_km=2):
    ...
    for poi_type in poi_types:
        c.execute("SELECT name, lat, lon FROM points_of_interest WHERE type = ?", (poi_type,))
